...
@Import(ProfissionalControllerTest.TestConfig.class)
class ProfissionalControllerTest {
    ...
    private static final String BASE_URL = "/spacovip/profissionais";

    private ProfissionalRequestDTO criarRequestValido() {
    ...
    private ProfissionalResponseDTO criarResponseValido(UUID id, List<UUID> servicosIds) {
    ...
    @Nested
    @DisplayName("POST /spacovip/profissionais - Cadastrar Profissional")
    class CadastrarProfissional {

        @Test
        @DisplayName("Deve retornar 201 Created com profissional criado")
        void deveRetornar201ComProfissionalCriado() throws Exception {
            ProfissionalRequestDTO request = criarRequestValido();
            ProfissionalResponseDTO response = criarResponseValido(UUID_FIXO, List.of());

            when(service.cadastrar(any(ProfissionalRequestDTO.class))).thenReturn(response);
            ...
            mockMvc.perform(post(BASE_URL)
                    ...
                    .andExpect(jsonPath("$.servicosIds").isEmpty());

            verify(service, times(1)).cadastrar(any(ProfissionalRequestDTO.class));
        }

        @Test
        @DisplayName("Deve retornar 201 Created com profissional e serviços vinculados")
        void deveRetornar201ComProfissionalEServicosVinculados() throws Exception {
            ...
            ProfissionalResponseDTO response = criarResponseValido(UUID_FIXO, List.of(SERVICO_ID_1, SERVICO_ID_2));
            ...
            mockMvc.perform(post(BASE_URL)
                    ...
                    .andExpect(status().isCreated())
                    .andExpect(jsonPath("$.servicosIds").isArray())
                    .andExpect(jsonPath("$.servicosIds", hasSize(2)));
        ...
        void deveRetornar400QuandoJsonInvalido() throws Exception {
            String jsonInvalido = "{ \"nome\": \"João\", \"email\": \"invalido\", \"telefone\": \"123\", \"dataNascimento\": \"10/05/1990\", \"sexo\": \"MASCULINO\" }";
            ...
            verify(service, never()).cadastrar(any(ProfissionalRequestDTO.class));
        }
        ...
        void deveRetornar409QuandoEmailJaExiste() throws Exception {
            ...
            when(service.cadastrar(any(ProfissionalRequestDTO.class)))
    ...
    @Nested
    @DisplayName("GET /spacovip/profissionais - Listar Profissionais")
    class ListarProfissionais {

        @Test
        @DisplayName("Deve retornar 200 OK com página de profissionais")
        void deveRetornar200ComPaginaDeProfissionais() throws Exception {
            ProfissionalResponseDTO response = criarResponseValido(UUID_FIXO, List.of());
            Page<ProfissionalResponseDTO> page = new PageImpl<>(List.of(response), PageRequest.of(0, 10), 1);

            when(service.buscarTodosProfissionais(any(Pageable.class))).thenReturn(page);
            ...
            verify(service, times(1)).buscarTodosProfissionais(any(Pageable.class));
        }

        @Test
        @DisplayName("Deve retornar 200 OK com página vazia quando não há profissionais")
        void deveRetornar200ComPaginaVazia() throws Exception {
            Page<ProfissionalResponseDTO> pageVazia = Page.empty();
            when(service.buscarTodosProfissionais(any(Pageable.class))).thenReturn(pageVazia);
        ...
        void deveAplicarPaginacaoEOrdenacaoPersonalizadas() throws Exception {
            Page<ProfissionalResponseDTO> page = Page.empty();
            when(service.buscarTodosProfissionais(any(Pageable.class))).thenReturn(page);
            ...
            verify(service, times(1)).buscarTodosProfissionais(argThat(p ->
                    p.getPageNumber() == 1 &&
                    p.getPageSize() == 5 &&
    ...
    @Nested
    @DisplayName("GET /spacovip/profissionais/{id} - Buscar Profissional por ID")
    class BuscarProfissionalPorId {

        @Test
        @DisplayName("Deve retornar 200 OK com profissional encontrado")
        void deveRetornar200ComProfissionalEncontrado() throws Exception {
            ProfissionalResponseDTO response = criarResponseValido(UUID_FIXO, List.of());
        ...
        @Test
        @DisplayName("Deve retornar 404 Not Found quando profissional não existe")
        void deveRetornar404QuandoProfissionalNaoExiste() throws Exception {
            UUID idInexistente = UUID.fromString("dddddddd-dddd-dddd-dddd-dddddddddddd");
            when(service.buscarPorId(idInexistente))
                    .thenThrow(new ResourceNotFoundException("Profissional não encontrado com o ID: " + idInexistente));

            mockMvc.perform(get(BASE_URL + "/" + idInexistente))
                    .andExpect(status().isNotFound())
                    .andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.erro").value("Profissional não encontrado com o ID: " + idInexistente));
        }
    }

    @Nested
    @DisplayName("PUT /spacovip/profissionais/{id} - Atualizar Profissional")
    class AtualizarProfissional {

        @Test
        @DisplayName("Deve retornar 200 OK com profissional atualizado")
        void deveRetornar200ComProfissionalAtualizado() throws Exception {
            ProfissionalRequestDTO request = new ProfissionalRequestDTO(
                    "João Santos",
            ...
            ProfissionalResponseDTO response = new ProfissionalResponseDTO(
                    UUID_FIXO, "João Santos", "Barbeiro especialista", "joao.novo@email.com", "11999999999",
                    DATA_NASCIMENTO_FIXA, Status.ATIVO, Sexo.MASCULINO, DATA_CADASTRO_FIXA, DATA_ATUALIZACAO_FIXA, List.of()
            );

            when(service.atualizar(eq(UUID_FIXO), any(ProfissionalRequestDTO.class))).thenReturn(response);
            ...
            verify(service, times(1)).atualizar(eq(UUID_FIXO), any(ProfissionalRequestDTO.class));
        }
        ...
        void deveRetornar404QuandoIdNaoExisteAoAtualizar() throws Exception {
            ...
            when(service.atualizar(eq(idInexistente), any(ProfissionalRequestDTO.class)))
                    .thenThrow(new ResourceNotFoundException("Profissional não encontrado com o ID: " + idInexistente));
        ...
        void deveRetornar400QuandoJsonInvalidoAoAtualizar() throws Exception {
            String jsonInvalido = "{ \"nome\": \"\", \"email\": \"invalido\", \"telefone\": \"123\", \"dataNascimento\": \"10/05/1990\", \"sexo\": \"MASCULINO\" }";
        ...
        @Test
        @DisplayName("Deve retornar 409 Conflict quando email de outro profissional")
        void deveRetornar409QuandoEmailDeOutroProfissional() throws Exception {
            ProfissionalRequestDTO request = criarRequestValido();

            when(service.atualizar(eq(UUID_FIXO), any(ProfissionalRequestDTO.class)))
                    .thenThrow(new ConflitoUnicidadeException("Este e-mail não está disponível."));
        ...
        @Test
        @DisplayName("Deve retornar 409 Conflict quando telefone de outro profissional")
        void deveRetornar409QuandoTelefoneDeOutroProfissional() throws Exception {
            ProfissionalRequestDTO request = criarRequestValido();

            when(service.atualizar(eq(UUID_FIXO), any(ProfissionalRequestDTO.class)))
                    .thenThrow(new ConflitoUnicidadeException("Este telefone não está disponível."));
    ...
    @Nested
    @DisplayName("PUT /spacovip/profissionais/{id}/desativar - Desativar Profissional")
    class DesativarProfissional {
        ...
        void deveRetornar404QuandoIdNaoExisteAoDesativar() throws Exception {
            UUID idInexistente = UUID.fromString("dddddddd-dddd-dddd-dddd-dddddddddddd");
            doThrow(new ResourceNotFoundException("Profissional não encontrado com o ID: " + idInexistente))
    ...
    @Nested
    @DisplayName("DELETE /spacovip/profissionais/{id} - Excluir Profissional")
    class ExcluirProfissional {
        ...
        void deveRetornar404QuandoIdNaoExisteAoExcluir() throws Exception {
            UUID idInexistente = UUID.fromString("dddddddd-dddd-dddd-dddd-dddddddddddd");
            doThrow(new ResourceNotFoundException("Profissional não encontrado para exclusão com o ID: " + idInexistente))
    ...
    class CasosDeBorda {
        ...
        void deveAceitarDataFormatoBrasileiro() throws Exception {
            ...
            ProfissionalResponseDTO response = new ProfissionalResponseDTO(
                    UUID_FIXO, "Maria Silva", "Manicure", "maria@email.com", "11912345678",
                    DATA_NASCIMENTO_FIXA, Status.ATIVO, Sexo.FEMININO, DATA_CADASTRO_FIXA, DATA_ATUALIZACAO_FIXA, List.of()
            );
            when(service.cadastrar(any(ProfissionalRequestDTO.class))).thenReturn(response);
